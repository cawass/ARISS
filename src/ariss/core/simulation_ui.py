...
NASA_GRID_MID = PALETTE["mid_grid"]
NASA_TEXT = PALETTE["primary_text"]
NASA_TEXT_SECONDARY = PALETTE["secondary_text"]
NASA_TEXT_MUTED = PALETTE["muted_text"]
NASA_SPINE = "#444444"
...
NASA_LINE = [PALETTE["l1_teal"], PALETTE["sernn_pink"], PALETTE["goal_dark"], PALETTE["choice_mid"], PALETTE["cat_purple"], PALETTE["cat_green"]]
GEOM_BODY, GEOM_INTAKE, GEOM_SOLAR, GEOM_RAD, GEOM_PROP, GEOM_EDGE = "#F5F7F8", "#EEF3F5", PALETTE["l1_teal"], "#F8F9FA", PALETTE["sernn_pink"], PALETTE["edge_gray"]
WAKE_CMAP, WAKE_NORM = cm.get_cmap("viridis"), colors.Normalize(vmin=0.0, vmax=1.0)
...
# --------------------------------------------------------------------------------------
# Small data structures / generic helpers
# --------------------------------------------------------------------------------------
...
def run_sizing_with_history(sc: SpacecraftState, max_iterations: int = 200, mass_tolerance: float = 1.0e-3) -> tuple[SpacecraftState, bool, list[SpacecraftState]]:
...
def _style_legend_text(legend) -> None:
    if legend:
        for text in legend.get_texts():
            text.set_color(NASA_TEXT_SECONDARY)


def _resolve_spacecraft_input(sc: SpacecraftInput | None) -> SpacecraftState:
    return load_spacecraft_from_base_config() if sc is None else sc if isinstance(sc, SpacecraftState) else load_spacecraft_from_base_config(Path(sc))
...
def _normalize_paths(path: str | Sequence[str] | None, available_paths: list[str]) -> list[str]:
    if not available_paths:
        return []
    candidates = [path] if isinstance(path, str) else list(path or [])
    selected = [candidate for candidate in candidates if candidate in available_paths]
    return selected or [("mass.Mass_total" if "mass.Mass_total" in available_paths else available_paths[0])]
...
def _flatten_numeric(prefix: str, value: Any, out: dict[str, float]) -> None:
    if is_dataclass(value):
        known_fields = {field.name for field in fields(value)}
        for name in known_fields:
            _flatten_numeric(f"{prefix}.{name}" if prefix else name, getattr(value, name), out)
        for name, child in vars(value).items():
            if name not in known_fields:
                _flatten_numeric(f"{prefix}.{name}" if prefix else name, child, out)
        return
...
def _history_series(history: list[SpacecraftState]) -> dict[str, list[float]]:
    ...
    for index, state in enumerate(history):
        ...
        for key, value in current.items():
            series.setdefault(key, [float("nan")] * index).append(value)
...
def _rect_dims(area: float, aspect_ratio: float) -> tuple[float, float]:
    area, aspect_ratio = _positive(area), _positive(aspect_ratio)
...
def _mounted_axial_bounds(center_from_aft: float, length: float, mount_length: float) -> tuple[float, float]:
    length, half_length, center = _positive(length), 0.5 * _positive(length), float(np.clip(center_from_aft, 0.0, _positive(mount_length)))
...
def _ellipse_radii(area: float, aspect_ratio: float) -> tuple[float, float]:
    ...
    return semi_y, area / (pi * semi_y)


def _is_square(shape: str) -> bool:
    normalized = (shape or "").strip().lower()
    return normalized.startswith(("s", "r"))
...
def _section_dims(area: float, aspect_ratio: float, shape_code: str) -> tuple[float, float]:
    ...
    return float(aspect_ratio * height), float(height)


def _local_section(x: float, body_length: float, intake_length: float, body_width: float, body_height: float, body_power: float, intake_width: float, intake_height: float, intake_power: float) -> tuple[float, float, float]:
    ...
    return tuple(float(v) for v in (body_width + progress * (intake_width - body_width), body_height + progress * (intake_height - body_height), body_power + progress * (intake_power - body_power)))


def _superellipse_half_span(coord: np.ndarray | float, semi_primary: float, semi_other: float, power: float) -> np.ndarray:
    coord_array, out = np.asarray(coord, dtype=float), np.zeros_like(np.asarray(coord, dtype=float))
    if semi_primary <= 0.0 or semi_other <= 0.0 or power <= 0.0:
        return out
    ratio, inside = np.abs(coord_array) / semi_primary, (np.abs(coord_array) / semi_primary) <= 1.0
...
def _interpolate_shape(start: _SectionShape, end: _SectionShape, progress: float) -> _SectionShape:
    ...
    return _SectionShape(lerp(start.width, end.width), lerp(start.height, end.height), lerp(start.semi_y, end.semi_y), lerp(start.semi_z, end.semi_z), start.is_square and end.is_square)


def _shape_at_x(x_geom: float, body_length: float, intake_length: float, body_shape: _SectionShape, intake_shape: _SectionShape) -> _SectionShape:
    return body_shape if intake_length <= 0.0 or x_geom <= body_length else _interpolate_shape(body_shape, intake_shape, (x_geom - body_length) / intake_length)
...
def _style_2d_axis(axis) -> None:
    ...
    for side in ("left", "bottom"):
        axis.spines[side].set_color(NASA_SPINE)
        axis.spines[side].set_linewidth(0.8)


def _box_faces(x0: float, x1: float, y0: float, y1: float, z0: float, z1: float) -> list[list[tuple[float, float, float]]]:
    return [
...
def _add_poly3d(axis, faces: Sequence[Sequence[Sequence[float]]], color: Any, alpha: float = 0.75, edgecolor: str = NASA_TEXT, linewidth: float = 0.9, zorder: float | None = None) -> None:
    collection = Poly3DCollection(faces, facecolors=color, edgecolors=edgecolor, linewidths=linewidth, alpha=alpha)
...
def _add_box(axis, x0: float, x1: float, y0: float, y1: float, z0: float, z1: float, color: Any, alpha: float = 0.75, edgecolor: str = NASA_TEXT, linewidth: float = 0.9, zorder: float | None = None) -> None:
    _add_poly3d(axis, _box_faces(x0, x1, y0, y1, z0, z1), color, alpha, edgecolor, linewidth, zorder)


def _add_box_edges(axis, x0: float, x1: float, y0: float, y1: float, z0: float, z1: float, color: str, linewidth: float = 1.3, zorder: float | None = None) -> None:
    vertices = [(x0, y0, z0), (x1, y0, z0), (x1, y1, z0), (x0, y1, z0), (x0, y0, z1), (x1, y0, z1), (x1, y1, z1), (x0, y1, z1)]
    edges = [(0, 1), (1, 2), (2, 3), (3, 0), (4, 5), (5, 6), (6, 7), (7, 4), (0, 4), (1, 5), (2, 6), (3, 7)]
    for start, end in edges:
        axis.plot([vertices[start][0], vertices[end][0]], [vertices[start][1], vertices[end][1]], [vertices[start][2], vertices[end][2]], color=color, linewidth=linewidth, zorder=zorder)


def _add_tapered_box(axis, x0: float, x1: float, front_width: float, front_height: float, rear_width: float, rear_height: float, color: Any, alpha: float = 0.75, edgecolor: str = NASA_TEXT, linewidth: float = 0.9, zorder: float | None = None) -> None:
    f0, f1, f2, f3 = (x0, -0.5 * front_width, -0.5 * front_height), (x0, 0.5 * front_width, -0.5 * front_height), (x0, 0.5 * front_width, 0.5 * front_height), (x0, -0.5 * front_width, 0.5 * front_height)
    r0, r1, r2, r3 = (x1, -0.5 * rear_width, -0.5 * rear_height), (x1, 0.5 * rear_width, -0.5 * rear_height), (x1, 0.5 * rear_width, 0.5 * rear_height), (x1, -0.5 * rear_width, 0.5 * rear_height)
    _add_poly3d(axis, [[f0, f1, f2, f3], [r0, r1, r2, r3], [f0, f1, r1, r0], [f1, f2, r2, r1], [f2, f3, r3, r2], [f3, f0, r0, r3]], color, alpha, edgecolor, linewidth, zorder)


def _add_tapered_elliptic_tube(axis, x0: float, length: float, front_semi_y: float, front_semi_z: float, rear_semi_y: float, rear_semi_z: float, color: Any, alpha: float = 0.75, edgecolor: str = NASA_TEXT, linewidth: float = 0.3, zorder: float | None = None) -> None:
    theta, x = np.linspace(0.0, 2.0 * np.pi, 40), np.linspace(x0, x0 + length, 20)
    progress = np.linspace(0.0, 1.0, len(x))
    semi_y, semi_z = front_semi_y + (rear_semi_y - front_semi_y) * progress, front_semi_z + (rear_semi_z - front_semi_z) * progress
    theta_grid, x_grid = np.meshgrid(theta, x, indexing="ij")
    y_grid, z_grid = np.tile(semi_y, (len(theta), 1)) * np.cos(theta_grid), np.tile(semi_z, (len(theta), 1)) * np.sin(theta_grid)
    surface = axis.plot_surface(x_grid, y_grid, z_grid, color=color, alpha=alpha, linewidth=linewidth, edgecolor=edgecolor, shade=False)
    if zorder is not None:
...
def _add_tapered_top_face(axis, x0: float, x1: float, front_width: float, rear_width: float, front_height: float, rear_height: float, color: Any, alpha: float = 0.92, edgecolor: str = "none", linewidth: float = 0.4, zorder: float | None = None) -> None:
    epsilon, front_z, rear_z = 1.0e-3 * max(front_height, rear_height, 1.0), 0.5 * front_height, 0.5 * rear_height
    _add_poly3d(axis, [[(x0, -0.5 * front_width, front_z + epsilon), (x0, 0.5 * front_width, front_z + epsilon), (x1, 0.5 * rear_width, rear_z + epsilon), (x1, -0.5 * rear_width, rear_z + epsilon)]], color, alpha, edgecolor, linewidth, zorder)


def _add_tapered_elliptic_shell(axis, x0: float, length: float, front_semi_y: float, front_semi_z: float, rear_semi_y: float, rear_semi_z: float, theta_min: float, theta_max: float, color: Any, alpha: float = 0.9, edgecolor: str = "none", linewidth: float = 0.0, zorder: float | None = None) -> None:
    theta, x = np.linspace(theta_min, theta_max, 32), np.linspace(x0, x0 + length, 20)
    progress = np.linspace(0.0, 1.0, len(x))
    semi_y, semi_z = front_semi_y + (rear_semi_y - front_semi_y) * progress, front_semi_z + (rear_semi_z - front_semi_z) * progress
    theta_grid, x_grid = np.meshgrid(theta, x, indexing="ij")
    y_grid, z_grid = np.tile(semi_y, (len(theta), 1)) * np.cos(theta_grid), np.tile(semi_z, (len(theta), 1)) * np.sin(theta_grid)
...
def _add_rectangle_outline(axis, x: float, width: float, height: float, color: Any, zorder: float | None = None) -> None:
    y0, y1, z0, z1 = -0.5 * width, 0.5 * width, -0.5 * height, 0.5 * height
    axis.plot([x, x, x, x, x], [y0, y1, y1, y0, y0], [z0, z0, z1, z1, z0], color=color, linewidth=2.0, zorder=zorder)


def _extend_bounds(bounds: dict[str, list[float]], x_values: Sequence[float], y_values: Sequence[float], z_values: Sequence[float]) -> None:
...
def _set_equal_limits(axis, bounds: dict[str, list[float]], padding: float = 0.05) -> None:
    x_min, x_max, y_min, y_max, z_min, z_max = min(bounds["x"]), max(bounds["x"]), min(bounds["y"]), max(bounds["y"]), min(bounds["z"]), max(bounds["z"])
    x_span, y_span, z_span = max(x_max - x_min, 1.0e-9), max(y_max - y_min, 1.0e-9), max(z_max - z_min, 1.0e-9)
    x_mid, y_mid, z_mid = 0.5 * (x_min + x_max), 0.5 * (y_min + y_max), 0.5 * (z_min + z_max)
...
def _style_3d_axis(axis, title: str) -> None:
    ...
    for pane in (axis.xaxis.pane, axis.yaxis.pane, axis.zaxis.pane):
        pane.set_facecolor(colors.to_rgba(NASA_BG, 1.0))
        pane.set_edgecolor(NASA_GRID)
    for ax in (axis.xaxis, axis.yaxis, axis.zaxis):
        try:
            ax._axinfo["grid"]["color"] = grid_color
            ax._axinfo["axisline"]["color"] = colors.to_rgba(NASA_SPINE, 1.0)
            ax._axinfo["tick"]["color"] = colors.to_rgba(NASA_TEXT_SECONDARY, 1.0)
        except Exception:
            pass
...
# Wake / drag helpers
# --------------------------------------------------------------------------------------
def _wake_profile_arrays(total_length: float, geometry: GeometryState, x_values: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if x_values is None or np.asarray(x_values, dtype=float).size == 0:
        x_drag = np.array([0.0, total_length], dtype=float) if total_length > 0.0 else np.array([0.0], dtype=float)
    else:
        x_drag = np.unique(np.sort(np.clip(np.asarray(x_values, dtype=float), 0.0, total_length)))
    body_end, wake_body, wake_in = float(np.clip(_safe_float(geometry.L_body), 0.0, total_length)), float(getattr(geometry, "wake_body", 1.0)), float(getattr(geometry, "wake_in", 1.0))
    profile = np.where(x_drag < body_end, wake_body, wake_in).astype(float)
    return x_drag, profile.copy(), profile.copy()


def _wake_fraction_at_x(x_geom: float, total_length: float, x_drag: np.ndarray, fy_profile: np.ndarray, fz_profile: np.ndarray) -> tuple[float, float, float]:
    if total_length <= 0.0 or x_drag.size == 0:
        return 1.0, 1.0, 1.0
    x_drag_value = float(np.clip(x_geom, 0.0, total_length))
    fy, fz = float(np.interp(x_drag_value, x_drag, fy_profile)), float(np.interp(x_drag_value, x_drag, fz_profile))
    return fy, fz, 0.5 * (fy + fz)


def _wake_color(value: float) -> tuple[float, float, float, float]:
    return WAKE_CMAP(WAKE_NORM(float(np.clip(np.nan_to_num(value, nan=0.0, posinf=1.0, neginf=0.0), 0.0, 1.0))))


def _drag_geometry_samples(body_length: float, intake_length: float, max_segments: int = 80) -> np.ndarray:
...
def _plot_drag_slice_lines(axis, geometry: GeometryState, n_x: int = 140, n_cuts: int = 7) -> None:
    ...
    p_body, p_in = _shape_power(geometry.S_body), _shape_power(geometry.S_in)
    max_half_width, max_half_height = 0.5 * max(w_body, w_in), 0.5 * max(h_body, h_in)
    if max_half_width <= 0.0 or max_half_height <= 0.0:
        return

    x_values, y_cuts, z_cuts = np.linspace(0.0, total_length, n_x, dtype=float), np.linspace(-0.9 * max_half_width, 0.9 * max_half_width, n_cuts, dtype=float), np.linspace(-0.9 * max_half_height, 0.9 * max_half_height, n_cuts, dtype=float)

    for y_cut in y_cuts:
        z_plus, z_minus = np.full_like(x_values, np.nan), np.full_like(x_values, np.nan)
        for i, x_value in enumerate(x_values):
            w_loc, h_loc, p_loc = _local_section(x_value, geometry.L_body, geometry.L_in, w_body, h_body, p_body, w_in, h_in, p_in)
            half_width, half_height = 0.5 * w_loc, 0.5 * h_loc
            ...
            z_plus[i], z_minus[i] = z_value, -z_value
        y_values = np.full_like(x_values, y_cut)
        for z_values in (z_plus, z_minus):
            valid = np.isfinite(z_values)
            if np.any(valid):
                axis.plot(x_values[valid], y_values[valid], z_values[valid], color=PALETTE["sernn_pink"], linewidth=0.9, alpha=0.55, zorder=42.0)

    for z_cut in z_cuts:
        y_plus, y_minus = np.full_like(x_values, np.nan), np.full_like(x_values, np.nan)
        for i, x_value in enumerate(x_values):
            w_loc, h_loc, p_loc = _local_section(x_value, geometry.L_body, geometry.L_in, w_body, h_body, p_body, w_in, h_in, p_in)
            half_width, half_height = 0.5 * w_loc, 0.5 * h_loc
            ...
            y_plus[i], y_minus[i] = y_value, -y_value
        z_values = np.full_like(x_values, z_cut)
        for y_values in (y_plus, y_minus):
            valid = np.isfinite(y_values)
            if np.any(valid):
                axis.plot(x_values[valid], y_values[valid], z_values[valid], color=PALETTE["goal_dark"], linewidth=0.9, alpha=0.55, zorder=42.0)


# --------------------------------------------------------------------------------------
# 3D spacecraft rendering
# --------------------------------------------------------------------------------------
def _add_symmetric_panel_pair(axis, *, x0: float, x1: float, lateral_extent: float, normal_thickness: float, offset: float, along_axis: str, color: Any, edgecolor: str, alpha: float, face_zorder: float, edge_zorder: float, bounds: dict[str, list[float]]) -> None:
    pos0, pos1, neg1, neg0 = offset, offset + lateral_extent, -offset, -(offset + lateral_extent)
    if along_axis == "y":
...
def _render_spacecraft(axis, geometry: GeometryState, color_fn: Callable[[str, float, float], Any], x_samples: np.ndarray | None = None, core_edgecolor: str | None = None) -> dict[str, list[float]]:
    bounds = {"x": [], "y": [], "z": []}
    body_length, intake_length, total_length = _positive(geometry.L_body, 0.1), _positive(geometry.L_in, 0.1), _positive(geometry.L_body, 0.1) + _positive(geometry.L_in, 0.1)
    body_shape, intake_shape = _shape_from_geometry(geometry.A_body, geometry.AR_body, geometry.S_body), _shape_from_geometry(geometry.A_in, geometry.AR_in, geometry.S_in)

    if x_samples is None:
        x_samples = np.array([0.0, body_length, total_length], dtype=float)
    else:
        x_samples = np.unique(np.clip(np.asarray(x_samples, dtype=float), 0.0, total_length))
        if x_samples.size < 2:
            x_samples = np.array([0.0, body_length, total_length], dtype=float)
    ...
    for x0, x1 in zip(x_samples[:-1], x_samples[1:]):
        if x1 <= x0:
            continue
        x_mid, shape_0, shape_1 = 0.5 * (x0 + x1), _shape_at_x(x0, body_length, intake_length, body_shape, intake_shape), _shape_at_x(x1, body_length, intake_length, body_shape, intake_shape)
        segment_color, zorder = color_fn("core", x_mid, total_length), 6.0 if x_mid >= body_length else 5.0
        segment_is_square = body_shape.is_square if x_mid <= body_length else intake_shape.is_square

        if segment_is_square:
            _add_tapered_box(axis, x0, x1, shape_0.width, shape_0.height, shape_1.width, shape_1.height, segment_color, alpha=0.95, edgecolor=core_edgecolor or "none", linewidth=0.8 if core_edgecolor else 0.0, zorder=zorder)
        else:
            _add_tapered_elliptic_tube(axis, x0, x1 - x0, shape_0.semi_y, shape_0.semi_z, shape_1.semi_y, shape_1.semi_z, segment_color, alpha=0.95, edgecolor=core_edgecolor or "none", linewidth=0.35 if core_edgecolor else 0.0, zorder=zorder)

        if geometry.A_solar > 0.0:
            solar_cover_color, solar_zorder = color_fn("solar", x_mid, total_length), zorder + 3.0
            if segment_is_square:
                _add_tapered_top_face(axis, x0, x1, shape_0.width, shape_1.width, shape_0.height, shape_1.height, solar_cover_color, alpha=0.95, edgecolor=GEOM_EDGE, linewidth=0.35, zorder=solar_zorder)
            else:
                _add_tapered_elliptic_shell(axis, x0, x1 - x0, 1.002 * shape_0.semi_y, 1.002 * shape_0.semi_z, 1.002 * shape_1.semi_y, 1.002 * shape_1.semi_z, 0.0, np.pi, solar_cover_color, alpha=0.9, edgecolor="none", linewidth=0.0, zorder=solar_zorder)

    max_width, max_height = max(body_shape.width, intake_shape.width), max(body_shape.height, intake_shape.height)
    _extend_bounds(bounds, [0.0, total_length], [-0.5 * max_width, 0.5 * max_width], [-0.5 * max_height, 0.5 * max_height])

    if geometry.A_prop > 0.0:
        prop_width, prop_height, prop_x = *_rect_dims(geometry.A_prop, geometry.AR_in), body_length + 0.95 * intake_length
    ...
    if geometry.A_solar > 0.0:
        ...
        solar_thickness, solar_clearance = max(0.06, 0.06 * min(body_shape.width, body_shape.height)), max(0.04, 1.25 * max(0.06, 0.06 * min(body_shape.width, body_shape.height)))
        solar_x0, solar_x1 = _mounted_axial_bounds(geometry.X_solar, solar_chord, body_length)
        _add_symmetric_panel_pair(axis, x0=solar_x0, x1=solar_x1, lateral_extent=solar_span, normal_thickness=solar_thickness, offset=0.5 * body_shape.width + solar_clearance, along_axis="y", color=color_fn("solar", 0.5 * (solar_x0 + solar_x1), total_length), edgecolor=NASA_TEXT, alpha=0.92, face_zorder=30.0, edge_zorder=31.0, bounds=bounds)
    ...
    if geometry.A_rad > 0.0:
        ...
        rad_thickness, rad_clearance = max(0.02, 0.03 * min(body_shape.width, body_shape.height)), max(0.03, 1.1 * max(0.02, 0.03 * min(body_shape.width, body_shape.height)))
        rad_x0, rad_x1 = _mounted_axial_bounds(geometry.X_rad, rad_chord, body_length)
        _add_symmetric_panel_pair(axis, x0=rad_x0, x1=rad_x1, lateral_extent=rad_span, normal_thickness=rad_thickness, offset=0.5 * body_shape.height + rad_clearance, along_axis="z", color=color_fn("rad", 0.5 * (rad_x0 + rad_x1), total_length), edgecolor=GEOM_EDGE, alpha=0.90, face_zorder=20.0, edge_zorder=21.0, bounds=bounds)
...
def draw_spacecraft_geometry(axis, geometry: GeometryState, iteration: int | None = None) -> None:
    ...
    def _color(component: str, x_mid: float, _total_length: float) -> Any:
        return GEOM_BODY if component == "core" and x_mid <= body_length else GEOM_INTAKE if component == "core" else GEOM_SOLAR if component == "solar" else GEOM_RAD if component == "rad" else GEOM_PROP if component == "prop" else GEOM_BODY
...
def draw_spacecraft_drag_geometry(figure, state: SpacecraftState, iteration: int | None = None) -> None:
    figure.clear()
    axis = figure.add_subplot(111, projection="3d")
    body_length, intake_length, total_length = max(_safe_float(state.geometry.L_body), 0.0), max(_safe_float(state.geometry.L_in), 0.0), max(_safe_float(state.geometry.L_body), 0.0) + max(_safe_float(state.geometry.L_in), 0.0)
    ...
    def _color(_component: str, x_mid: float, _total: float) -> Any:
        return _wake_color(_wake_fraction_at_x(x_mid, total_length, x_drag, fy_profile, fz_profile)[2])
...
def _momentum_exchange_terms(state: SpacecraftState) -> dict[str, float]:
    rho, velocity = _safe_float(state.orbit.density), _safe_float(state.orbit.velocity)
    area_ref, area_prop, total_drag = _safe_float(state.geometry.A_ref), _safe_float(state.geometry.A_prop), _safe_float(state.drag.drag_total)
    refueling_exchange, propulsive_exchange = rho * area_ref * velocity * velocity, rho * area_prop * velocity * velocity
    total_exchange = refueling_exchange + propulsive_exchange
    return {"refueling_exchange": refueling_exchange, "propulsive_exchange": propulsive_exchange, "total_drag": total_drag, "total_exchange": total_exchange, "total_load": total_drag + total_exchange}


def _drag_component_label(attr_name: str) -> str:
    labels = {"drag_body_side": "Body Side", "drag_inlet_side": "Inlet Side", "drag_inlet_front": "Inlet Front", "drag_body": "Body", "drag_inlet": "Inlet", "drag_inlet_normal": "Inlet Normal", "drag_solar": "Solar", "drag_rad": "Radiator"}
...
def _drag_component_map(state: SpacecraftState) -> dict[str, float]:
    drag_values = {field_info.name: getattr(state.drag, field_info.name) for field_info in fields(state.drag)} if is_dataclass(state.drag) else {}
    drag_values.update(vars(state.drag))
    ...
    component_names.extend(sorted(name for name in drag_values if name.startswith("drag_") and name not in component_names and name != "drag_total"))
    return {name: _safe_float(drag_values[name]) for name in component_names}


def _thermal_component_label(attr_name: str) -> str:
    labels = {"Q_drag": "Drag", "Q_sun": "Sun", "Q_albedo": "Albedo", "Q_ir": "Earth IR", "Q_internal": "Internal", "Q_radiated": "Radiated"}
...
def _thermal_component_map(diagnostics: ThermalDiagnostics) -> dict[str, float]:
    return {name: _safe_float(getattr(diagnostics, name, 0.0)) for name in ["Q_drag", "Q_sun", "Q_albedo", "Q_ir", "Q_internal", "Q_radiated"]}


def draw_drag_distribution(figure, state: SpacecraftState, iteration: int | None = None) -> None:
    figure.clear()
    axes = figure.subplots(2, 2, squeeze=False)
    drag_coefficients = {"Body Side": _safe_float(state.drag.cd_body_side), "Inlet Side": _safe_float(state.drag.cd_inlet_side), "Inlet Front": _safe_float(state.drag.cd_inlet_front), "Solar": _safe_float(state.drag.cd_solar), "Radiator": _safe_float(state.drag.cd_rad)}
    drag_components = _drag_component_map(state)
    wake_factors = {"Body": _safe_float(getattr(state.geometry, "wake_body", 1.0)), "Inlet": _safe_float(getattr(state.geometry, "wake_in", 1.0)), "Solar": _safe_float(getattr(state.geometry, "wake_solar", 1.0)), "Radiator": _safe_float(getattr(state.geometry, "wake_radiator", 1.0))}
    exchange_terms = _momentum_exchange_terms(state)
    orbit_density, orbit_velocity = _safe_float(state.orbit.density), _safe_float(state.orbit.velocity)
    ...
    force_labels, force_values = [_drag_component_label(name) for name in drag_components], list(drag_components.values())
    force_axis.bar(force_labels, force_values, color=[NASA_LINE[i % len(NASA_LINE)] for i in range(len(force_labels))], edgecolor=NASA_TEXT, linewidth=0.8)
    shown_drag_sum, total_drag = float(np.sum(force_values)), exchange_terms["total_drag"]
    ...
    thrust_available, balance_residual = _safe_float(getattr(state.thruster, "thrust", 0.0)), _safe_float(getattr(state.thruster, "thrust", 0.0)) - exchange_terms["total_load"]
...
def draw_thermal_distribution(figure, state: SpacecraftState, diagnostics: ThermalDiagnostics, iteration: int | None = None) -> None:
    figure.clear()
    axes = figure.subplots(2, 2, squeeze=False)
    ...
    total_input, q_radiated, net_load = float(np.sum(input_values)), components["Q_radiated"], float(np.sum(input_values)) - components["Q_radiated"]
...
def draw_propulsion_overview(figure, state: SpacecraftState, iteration: int | None = None) -> None:
    ...
    power_required, thrust, mass_flow = _safe_float(getattr(state.thruster, "power", 0.0)), _safe_float(getattr(state.thruster, "thrust", 0.0)), _safe_float(getattr(state.thruster, "m_flow", 0.0))
    isp, efficiency, prop_area, intake_area = _safe_float(getattr(state.thruster, "specific_impulse", 0.0)), _safe_float(getattr(state.thruster, "eff", 0.0)), _safe_float(getattr(state.geometry, "A_prop", 0.0)), _safe_float(getattr(state.geometry, "A_in", 0.0))
    orbit_density, orbit_velocity = _safe_float(getattr(state.orbit, "density", 0.0)), _safe_float(getattr(state.orbit, "velocity", 0.0))
    exhaust_velocity, thrust_to_power = const.EARTH_GRAVITY * isp, (0.0 if power_required <= 0.0 else 1.0e6 * thrust / power_required)

    intake_radius = max(np.sqrt(max(intake_area, 1.0e-9) / np.pi), 0.16)
    throat_radius = max(np.sqrt(max(prop_area, 1.0e-9) / np.pi), 0.08)
    chamber_radius, exit_radius = max(throat_radius * 1.35, intake_radius * 0.62), max(throat_radius * (1.45 + 0.8 * efficiency), throat_radius * 1.6)
    intake_length, chamber_length, nozzle_length = max(0.45, 0.45 + 0.30 * intake_radius), max(0.9, 0.55 + 0.00006 * power_required + 0.25 * chamber_radius), max(1.0, 0.85 + 0.00002 * isp + 0.20 * exit_radius)

    x0, x1, x2, x3 = 0.0, intake_length, intake_length + chamber_length, intake_length + chamber_length + nozzle_length
    upper, lower, x_nodes = np.array([intake_radius, chamber_radius, throat_radius, exit_radius]), -np.array([intake_radius, chamber_radius, throat_radius, exit_radius]), np.array([x0, x1, x2, x3])

    silhouette_axis.set_facecolor(NASA_BG)
    silhouette_axis.fill_between(x_nodes, upper, lower, color=PALETTE["goal_light"], edgecolor=NASA_TEXT, linewidth=1.6)
...
class _HistoryPlotterUI:
    AUX_TAB_TITLES = {"geometry": "3D View", "drag": "Drag/Propulsion", "thermal": "Thermal", "atmosphere": "Atmosphere"}

    def __init__(self, history: list[SpacecraftState], series: dict[str, list[float]], default_specs: list[PlotSpec], window_title: str) -> None:
        if tk is None or ttk is None:
            raise RuntimeError("Tkinter is not available in this environment.")

        self.history, self.series, self.paths, self.rows = history, series, list(series.keys()), []
        ...
        self.atmosphere_profile_cache: dict[tuple[str, float, float, float, float, bool], dict[str, np.ndarray]] = {}
        self.root = tk.Tk()
    ...
    def _build_layout(self, default_specs: list[PlotSpec]) -> None:
        ...
        for key, title in self.AUX_TAB_TITLES.items():
            self.figure_tabs[key] = self._create_figure_tab(title, self.status_vars[key])
    ...
    def _build_controls(self, parent) -> None:
        ...
        tk.Button(button_row, text="Render", command=self.redraw, bg=NASA_PANEL, fg=NASA_TEXT, activebackground=PALETTE["sernn_pink_fill"], activeforeground=NASA_TEXT, relief="flat", bd=1, highlightthickness=1, highlightbackground=NASA_GRID).pack(side="left")
    ...
    def _configure_notebook_style(self) -> None:
        ...
        style.map(
            ...
            lightcolor=[("selected", PALETTE["l1_teal"])],
            darkcolor=[("selected", PALETTE["l1_teal"])],
        )
    ...
    def _create_history_tab(self) -> None:
        ...
        self.notebook.add(tab, text="History Plots")
        self.figure = plt.Figure(figsize=(11, 8), dpi=100, facecolor=NASA_BG)
        self.canvas, self.plot_toolbar = self._attach_figure_canvas(tab, self.figure)
    ...
    def _get_drag_state(self, index: int) -> SpacecraftState:
        if index not in self.drag_state_cache:
            with redirect_stdout(io.StringIO()):
                self.drag_state_cache[index] = compute_drag_diagnostics(self.history[index])
        return self.drag_state_cache[index]

    def _get_thermal_diagnostics(self, index: int) -> tuple[SpacecraftState, ThermalDiagnostics]:
        if index not in self.thermal_diagnostics_cache:
            state = deepcopy(self.history[index])
            self.thermal_diagnostics_cache[index] = (state, thermal_model(state))
        return self.thermal_diagnostics_cache[index]
    ...
    def add_plot_row(self, path: str | Sequence[str] | None = None, title: str | None = None, log_scale: bool = False) -> None:
        selected_paths, row_title = _normalize_paths(path, self.paths), title or _default_title(_normalize_paths(path, self.paths))
        ...
        title_var, log_var = tk.StringVar(value=row_title), tk.BooleanVar(value=log_scale)
        ...
        path_list = tk.Listbox(series_frame, selectmode=tk.MULTIPLE, exportselection=False, width=42, height=6, bg=NASA_PANEL, fg=NASA_TEXT, selectbackground=PALETTE["l1_teal_fill"], selectforeground=NASA_TEXT, activestyle="none", relief="flat", highlightthickness=1, highlightbackground=NASA_GRID)
    ...
    def redraw(self) -> None:
        ...
        plot_count, cols = len(self.rows), (1 if len(self.rows) == 1 else 2)
        row_count, axes = ceil(plot_count / cols), self.figure.subplots(ceil(plot_count / cols), cols, squeeze=False)
    ...
    def redraw_geometry(self) -> None:
        index, state = self._current_index(), self.history[self._current_index()]
        tab, figure = self.figure_tabs["geometry"], self.figure_tabs["geometry"]["figure"]
        figure.clear()
        draw_spacecraft_geometry(figure.add_subplot(111, projection="3d"), state.geometry, iteration=index)
        self.status_vars["geometry"].set(f"Iteration {index} / {len(self.history) - 1} | Mass {_safe_float(state.mass.Mass_total):.2f} kg | Altitude {_safe_float(state.orbit.altitude):.2f} km")
        figure.tight_layout()
        tab["canvas"].draw_idle()

    def redraw_drag(self) -> None:
        index, tab, figure = self._current_index(), self.figure_tabs["drag"], self.figure_tabs["drag"]["figure"]
        try:
            state = self._get_drag_state(index)
            draw_drag_distribution(figure, state, iteration=index)
            exchange_terms, drag_components = _momentum_exchange_terms(state), _drag_component_map(state)
            ...
            self.status_vars["drag"].set(f"Iteration {index} / {len(self.history) - 1}\n{component_summary}\nState total {exchange_terms['total_drag']:.3e} N | Shown sum {shown_drag_sum:.3e} N | Refuel exchange {exchange_terms['refueling_exchange']:.3e} N | Propulsive exchange {exchange_terms['propulsive_exchange']:.3e} N | Total load {exchange_terms['total_load']:.3e} N | Thrust {thrust_available:.3e} N | Residual {balance_residual:.3e} N")
    ...
    def redraw_thermal(self) -> None:
        index, tab, figure = self._current_index(), self.figure_tabs["thermal"], self.figure_tabs["thermal"]["figure"]
        try:
            state, diagnostics = self._get_thermal_diagnostics(index)
            draw_thermal_distribution(figure, state, diagnostics, iteration=index)
            components = _thermal_component_map(diagnostics)
            self.status_vars["thermal"].set(f"Iteration {index} / {len(self.history) - 1} | Drag {components['Q_drag']:.3e} W | Sun {components['Q_sun']:.3e} W | Albedo {components['Q_albedo']:.3e} W | IR {components['Q_ir']:.3e} W | Internal {components['Q_internal']:.3e} W | Radiated {components['Q_radiated']:.3e} W | A_rad {_safe_float(state.geometry.A_rad):.3f} m^2")
        except Exception as exc:
    ...
    def redraw_atmosphere(self) -> None:
        index, state, altitude_km = self._current_index(), self.history[self._current_index()], _safe_float(self.history[self._current_index()].orbit.altitude)
        tab, figure = self.figure_tabs["atmosphere"], self.figure_tabs["atmosphere"]["figure"]
        figure.clear()
        ...
        try:
            ...
            altitude_profile, total_density_profile = profile["altitude_km"], profile["total_density"]
            o2_profile, n2_profile, o_profile, r_profile = profile["o2_density"], profile["n2_density"], profile["o_density"], profile["r_specific"]
            model_density, state_density = max(_safe_float(properties["density"]), 1.0e-30), max(_safe_float(state.orbit.density, default=max(_safe_float(properties["density"]), 1.0e-30)), 1.0e-30)

            composition_axis.plot(altitude_profile, total_density_profile, color=NASA_TEXT, linewidth=1.8, linestyle="--", label="Total density")
            composition_axis.plot(altitude_profile, o2_profile, color=PALETTE["l1_teal"], linewidth=1.9, label="O2 density")
            composition_axis.plot(altitude_profile, n2_profile, color=PALETTE["cat_green"], linewidth=1.9, label="N2 density")
            composition_axis.plot(altitude_profile, o_profile, color=PALETTE["sernn_pink"], linewidth=1.9, label="O density")
            ...
            composition_axis.scatter([altitude_km], [state_density], color=PALETTE["goal_dark"], s=30, marker="D", zorder=5, label=f"State density = {state_density:.3e}")
            composition_axis.scatter([altitude_km], [max(_safe_float(properties["o2_density"]), 1.0e-30)], color=PALETTE["l1_teal"], s=30, zorder=5)
            ...
            self.status_vars["atmosphere"].set(f"Iteration {index} / {len(self.history) - 1} | Atmosphere diagnostics unavailable")
...
# Public entry points
# --------------------------------------------------------------------------------------
def launch_history_ui(sc: SpacecraftInput | None = None, max_iterations: int = 200, mass_tolerance: float = 1.0e-3, default_specs: list[PlotSpec] | None = None, window_title: str = "ARISS History Plotter", show: bool = True):
...
def _launch_with_specs(specs: list[PlotSpec], title: str, sc: SpacecraftInput | None = None, max_iterations: int = 200, mass_tolerance: float = 1.0e-3, show: bool = True):
    return launch_history_ui(sc=sc, max_iterations=max_iterations, mass_tolerance=mass_tolerance, default_specs=specs, window_title=title, show=show)


def plot_atmosphere_profiles(height_min_km: float = 80.0, height_max_km: float = 1000.0, samples: int = 600, show: bool = True):
    _ = (height_min_km, height_max_km, samples)
    return _launch_with_specs(ATMOSPHERE_SPECS, "ARISS Atmosphere / Orbit Console", show=show)


def plot_budgets_total(sc: SpacecraftInput | None = None, max_iterations: int = 20, mass_tolerance: float = 1.0e-8, show: bool = True):
    return _launch_with_specs(BUDGET_SPECS, "ARISS Budget Console", sc=sc, max_iterations=max_iterations, mass_tolerance=mass_tolerance, show=show)


def plot_dimension_evolution(sc: SpacecraftInput | None = None, max_iterations: int = 20, mass_tolerance: float = 1.0e-8, show: bool = True):
    return _launch_with_specs(DIMENSION_SPECS, "ARISS Geometry Console", sc=sc, max_iterations=max_iterations, mass_tolerance=mass_tolerance, show=show)


def plot_drag_diagnostics(sc: SpacecraftInput | None = None, n_points: int = 64, show: bool = True):
    _ = n_points
    return _launch_with_specs(DRAG_SPECS, "ARISS Drag Console", sc=sc, show=show)


def plot_power_diagnostics(sc: SpacecraftInput | None = None, efficiency: float = 0.2, alignment_deg: float = 0.0, baseline_power: float = 2000.0, show: bool = True):
    _ = (efficiency, alignment_deg, baseline_power)
    return _launch_with_specs(POWER_SPECS, "ARISS Power Console", sc=sc, show=show)


def plot_propulsion_diagnostics(sc: SpacecraftInput | None = None, baseline_drag: float = 0.2, show: bool = True):
    _ = baseline_drag
    return _launch_with_specs(PROPULSION_SPECS, "ARISS Propulsion Console", sc=sc, show=show)


def plot_simulation_budgets(sc: SpacecraftInput | None = None, max_iterations: int = 50, mass_tolerance: float = 1.0e-9, show: bool = True):
    return _launch_with_specs(SIM_BUDGET_SPECS, "ARISS Integrated Budget Console", sc=sc, max_iterations=max_iterations, mass_tolerance=mass_tolerance, show=show)


def plot_simulation_history(sc: SpacecraftInput | None = None, max_iterations: int = 200, mass_tolerance: float = 1.0e-3, show: bool = True):
    return _launch_with_specs(DEFAULT_HISTORY_SPECS, "ARISS Mission History Console", sc=sc, max_iterations=max_iterations, mass_tolerance=mass_tolerance, show=show)


__all__ = [
    "draw_spacecraft_drag_geometry", "draw_spacecraft_geometry", "launch_history_ui",
    "plot_atmosphere_profiles", "plot_budgets_total", "plot_dimension_evolution",
    "plot_drag_diagnostics", "plot_power_diagnostics", "plot_propulsion_diagnostics",
    "plot_simulation_budgets", "plot_simulation_history", "run_sizing_with_history",
]
