...
class OrbitState:
    ...
    msis_date: str = "2000-01-01T00:00:00"  # [ISO-8601] Reference UTC date used by the MSIS atmosphere call.
    msis_f107: float = 140.0  # [sfu] Solar 10.7 cm radio flux (daily value) used by MSIS.
    msis_ap: float = 15.0  # [-] Geomagnetic Ap index used by MSIS.
...
class GeometryState:
    ...
    AR_solar: float = 5  # [-] Solar-panel planform aspect ratio.
    AR_rad: float = 5  # [-] Radiator planform aspect ratio.
    ...
    wake_in: float = 1  # [-] Total inlet side exposed to atmospheric air
    wake_body: float = 1  # [-] Total body side exposed to atmospheric air
    wake_solar: float = 1 # [-] Total panel area exposed to atmospheric air
    wake_radiator: float = 1 # [-] Total radiator area expoused to atmospheric air
    ...
    X_solar: float = 2.0  # [m] Axial center of the solar panels measured from the thruster-side butt.
    X_rad: float = 0.5  # [m] Axial center of the radiator panels measured from the thruster-side butt.
...
class ThrusterState:
    ...
    eff: float = 0.5  # [-] Electrical-to-jet efficiency of the thruster.
    power: float = 5000.0  # [W] Electrical power supplied to the thruster.
...
@dataclass(frozen=True)
class ThermalState:
    T_des: float = 300.0  # [K] Design temperature used by thermal and refueling calculations.
